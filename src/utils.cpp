...
/*Ищет последнее слово , Если оно == DEBUG || INFO || ERROR ,
 присваивает соответствующий уровень собщению*/ 
...
MessageLevel parseLevel(const std::string& levelStr) {
    ...
    std::cerr << "Unknown level. Defaulting to INVALID\n";
    return MessageLevel::INVALID;
}
